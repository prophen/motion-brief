...
import {
  ...
  NARRATION_TARGET_MIN_WORDS,
} from '../lib/narration.js'
import {
  DEFAULT_MOTIONBRIEF_VOICE_ID,
  isMotionBriefVoiceId,
} from '../lib/voices.js'
import {
  assetFileExtension,
  decodeBase64DataUrl,
...
function parseCreativeBrief(content: string): CreativeBrief {
  ...
  for (const key of keys) {
    ...
    if (typeof field !== 'string' || !field.trim())
      throw new Error(`openai_brief_invalid_${key}`)
    result[key] = field.trim()
  }
  const narrationWords = countWords(result.narration)
  if (
    narrationWords < NARRATION_TARGET_MIN_WORDS ||
    narrationWords > NARRATION_HARD_MAX_WORDS
  ) {
...
export async function generateCreativeBrief(
  ...
  const response = await apiWorkerFetch(
    env,
    '/api/integrations/openai/chat-completion',
    {
      method: 'POST',
      headers: {
...
export async function runProviderDiagnostic(
  ...
  const response = await apiWorkerFetch(
    env,
    `/api/integrations/${input.endpoint}`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
export async function submitFalStill(
  env: Env,
  prompt: string,
): Promise<FalStillSubmission> {
  if (!prompt.trim()) throw new Error('still_prompt_required')
  const response = asObject(
    await callIntegration(env, 'fal/run-model', {
  ...
  if (typeof requestId !== 'string')
    throw new Error('fal_still_submission_missing_job_id')
  return { requestId }
}

export async function pollFalStill(
  env: Env,
  requestId: string,
): Promise<FalStillPoll> {
  const response = asObject(
    await callIntegration(env, 'fal/get-result', { request_id: requestId }),
  )
  const data = asObject(response?.data) ?? response
  const status =
  ...
  if (['failed', 'error', 'cancelled', 'canceled'].includes(status)) {
    return {
      status: 'failed',
      error:
        typeof data?.error === 'string' ? data.error : 'FAL generation failed',
    }
  }
...
export async function submitFalMotion(
  ...
  if (imageUrl.protocol !== 'https:')
    throw new Error('motion_image_must_use_https')
  const response = asObject(
    await callIntegration(env, 'fal/run-model', {
  ...
  const requestId = data?.jobId ?? data?.request_id
  if (typeof requestId !== 'string')
    throw new Error('fal_motion_submission_missing_job_id')
  return { requestId }
}
    ...
    return {
      ...
      error:
        typeof data?.error === 'string'
          ? data.error
          : 'FAL motion generation failed',
    }
  }
  const video = asObject(asObject(data?.output)?.video)
  if (typeof video?.url === 'string')
    return { status: 'complete', videoUrl: video.url }
  return {
    status: 'pending',
    queuePosition:
      typeof data?.queuePosition === 'number' ? data.queuePosition : undefined,
...
export async function generateNarrationDataUrl(
  env: Env,
  text: string,
  voiceId: string = DEFAULT_MOTIONBRIEF_VOICE_ID,
): Promise<string> {
  ...
  if (!isMotionBriefVoiceId(voiceId))
    throw new Error('narration_voice_not_allowed')
  const response = asObject(
    await callIntegration(env, 'elevenlabs/generate-speech', {
      text: text.trim(),
      voice_id: voiceId,
  ...
  if (typeof data?.audioUrl !== 'string')
    throw new Error('elevenlabs_response_missing_audio')
...
async function inspectPublicAsset(
  kind: AssetPreflight['kind'],
  url: string,
  signal?: AbortSignal,
...
export async function submitShotstackRender(
  ...
  console.info(
    '[motionbrief] submitting Shotstack edit',
    JSON.stringify({
      duration: edit.duration,
      tracks: edit.timeline.tracks.map((track) =>
        track.clips.map((value) => {
          const clip = value as {
            asset: { type: string; transcode?: boolean }
            start: number
            length: number
          }
          return {
            ...
            start: clip.start,
            length: clip.length,
            transcode: clip.asset.transcode,
          }
        }),
      ),
      output: edit.output,
  ...
  const response = asObject(
    await callIntegration(env, 'shotstack/render', edit),
  )
  const data = asObject(response?.data) ?? response
  if (typeof data?.id !== 'string')
    throw new Error('shotstack_submission_missing_render_id')
...
export async function pollShotstackRender(
  env: Env,
  renderId: string,
): Promise<ShotstackPoll> {
  const response = asObject(
    await callIntegration(env, 'shotstack/get-render', { id: renderId }),
  )
  const data = asObject(response?.data) ?? response
  const status =
...
export async function storeRemoteAsset(
  ...
  if (!env.APP_IDENTITY_TOKEN)
    throw new Error('app_identity_required_for_asset_storage')

  const source = new URL(input.sourceUrl)
  if (source.protocol !== 'https:')
    throw new Error('asset_source_must_use_https')
  const downloaded = await fetch(source, {
    signal: input.signal,
    redirect: 'follow',
  })
  if (!downloaded.ok)
    throw new Error(`asset_download_failed_${downloaded.status}`)
  const mimeType =
...
async function storeAssetBytes(
  ...
  form.append(
    ...
    key.split('/').at(-1) ?? 'asset',
  )
  form.append('name', key.split('/').at(-1) ?? 'asset')

  const uploaded = await platformWorkerFetch(
    env,
    new Request(url, {
      method: 'POST',
      headers: {
