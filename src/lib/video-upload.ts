...
export function validateUploadedMotion(input: { type: string; size: number; duration: number; width: number; height: number }): string | null {
  ...
  if (!Number.isFinite(input.duration) || input.duration < 4.95 || input.duration > 5.5) return 'Choose a video between 4.95 and 5.5 seconds.'
...
export function readMediaDuration(url: string, kind: 'audio' | 'video'): Promise<number> {
