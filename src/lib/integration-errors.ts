...
function collectMessages(
  ...
  if (Array.isArray(value))
    return value.forEach((child) =>
      collectMessages(child, messages, requestIds),
    )
  const object = asObject(value)
  if (!object) return
  for (const [key, child] of Object.entries(object)) {
    if (
      ['request_id', 'requestId', 'correlation_id'].includes(key) &&
      typeof child === 'string'
    )
      requestIds.push(child)
    if (
      ['error', 'message', 'detail'].includes(key) &&
      typeof child === 'string'
    ) {
      messages.push(child)
      ...
      if (jsonStart >= 0) {
        try {
          collectMessages(
            JSON.parse(child.slice(jsonStart)),
            messages,
            requestIds,
          )
        } catch {
...
export function formatIntegrationError(
  ...
  const useful =
    [...uniqueMessages]
      .reverse()
      .find((message) => !/^.+ API error \d+:/.test(message)) ??
    uniqueMessages.at(-1) ??
