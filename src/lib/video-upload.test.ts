...
import { MAX_UPLOADED_VIDEO_BYTES, validateUploadedMotion } from './video-upload'

describe('uploaded motion validation', () => {
  const valid = { type: 'video/mp4', size: 2_000_000, duration: 5, width: 1080, height: 1920 }

  it('accepts a five-second 9:16 MP4', () => expect(validateUploadedMotion(valid)).toBeNull())
  it('rejects wrong formats and oversized files', () => {
    expect(validateUploadedMotion({ ...valid, type: 'video/quicktime' })).toContain('MP4')
