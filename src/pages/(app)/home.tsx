...
export default function HomePage() {
  ...
  async function generateMotion(){ if(!isSignedIn||!recordId) return toast.info('Save first','Save the brief before generating motion.'); const recover=motionJob?.result?.temporaryVideoUrl; const imageKey=storedAssetKey(project,'image'); if(!recover&&!imageKey) return toast.error('Stored still required','Generate and store a still before animation.'); if(!recover&&!project.motionPrompt.trim()) return toast.error('Motion prompt missing','Generate or enter a motion prompt first.'); setQueuing(true); try{ if(recover){await enqueue('motionbrief-store-motion',{projectId:recordId,sourceUrl:recover},{maxAttempts:1}); toast.success('Video storage retry queued','Reusing the existing FAL video; no generation charge.')}else{await enqueue('motionbrief-generate-motion',{projectId:recordId,motionPrompt:project.motionPrompt,imageKey},{maxAttempts:1}); toast.success('Motion queued','Wan generates one silent five-second video with a $0.25 provider cap; estimated DeepSpace reservation is $0.325.')} }catch(e){toast.error('Could not queue',e instanceof Error?e.message:'Please try again.')}finally{setQueuing(false)} }
