...
export default function ApiStatusPage() {
  const catalog = useAsyncResource<IntegrationCatalog>(
    async (signal) => {
      const res = await integration.get<IntegrationCatalog>('', undefined, {
        signal,
      })
    ...
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-6 p-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          A scaffolded pattern for server-backed resources with explicit
      ...
      {catalog.error && catalog.data && (
        <div className="rounded-lg border border-warning/30 bg-warning/10 px-4 py-3">
          <p className="text-sm font-medium text-warning">
            Showing the last loaded catalog
          </p>
      ...
      {catalog.status === 'loading' ? (
        ...
        <div className="rounded-lg border border-destructive/30 bg-destructive/10 px-4 py-3">
          ...
          <div className="mt-1 flex flex-col gap-3 text-sm text-muted-foreground">
            ...
            {catalog.retryCount > 0 && (
              <span>
                Retried {catalog.retryCount} time
                {catalog.retryCount === 1 ? '' : 's'} automatically.
              </span>
            )}
            <button
              type="button"
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border border-border bg-card px-4 py-16 text-center">
          ...
          <p className="mt-2 max-w-sm text-sm text-muted-foreground">
            The catalog loaded, but it did not return any integration
            definitions.
          </p>
        </div>
      ) : (
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-base font-medium">
                Integration catalog ready
              </h2>
              <p className="mt-1 text-sm text-muted-foreground">
                {integrationCount} integration
                {integrationCount === 1 ? '' : 's'} available.
              </p>
            </div>
